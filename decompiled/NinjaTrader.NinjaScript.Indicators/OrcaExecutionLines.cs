...
public class OrcaExecutionLines : Indicator
{
	...
	private void UpdateLiveMAEMFE()
	{
		...
		lock (tradeLock)
		{
			foreach (KeyValuePair<string, AccountState> accountState in accountStates)
			{
				AccountState value = accountState.Value;
	...
	protected override void OnRender(ChartControl chartControl, ChartScale chartScale)
	{
		...
		{
			Brush wpfBrush = (item2.IsLong ? LongMarkerColor : ShortMarkerColor);
			...
			{
				SolidColorBrush val = ToD2DBrush(wpfBrush);
				try
				{
					DrawTriangle(item2.IsLong, x5, y5, 8f, val);
					DrawTriangle(!item2.IsLong, x6, y6, 8f, val);
				}
				finally
				{
			...
			SolidColorBrush val2 = ToD2DBrush(wpfBrush, 0.65f);
			try
			{
	...
	private void DrawHoverLabel(RoundTrip rt, FillMatch match, float exitX, float exitY)
	{
		...
		try
		{
			...
			TextFormat val3 = new TextFormat(Globals.DirectWriteFactory, "Arial", (FontWeight)700, (FontStyle)0, (float)LabelFontSize);
			try
			{
