...
| `OrcaSessionContextMap` | Yes | 30 Second; 1 Tick when session volume profile is enabled | Bid, Ask, Last; tick-series trade price/volume | Session state, 30-second opening range, true traded-at-price profile/VWAP, and delta updates | Moderate plus hidden tick-series load when profile is enabled | No shared provider found; no estimated bar-volume profile fallback | Explicit SharpDX `OnRender`; labels remain price-anchored and carry/projection labels are suppressed |
