...
| `OrcaHTFCandles` | Yes | One selected Minute 5/15/30/60/120/240 or Day 1; Minute 30 for custom Weekly, ETH/RTH, and Asia/London/New York | None | Constant-time native OHLC updates; custom modes aggregate one 30-minute source bar at a time | No Tick Replay requirement | No shared or local order-flow cache | SharpDX `OnRender` over a bounded immutable completed array plus one lock-copied active candle |
| `OrcaSessionContextMap` | Yes | 30 Second; 1 Tick when session volume profile is enabled | Bid, Ask, Last; tick-series trade price/volume | Session state, 30-second opening range, true traded-at-price profile/VWAP, and delta updates | Moderate plus hidden tick-series load when profile is enabled; profile summary recomputation is deferred to render-snapshot publication | No shared provider found; no estimated bar-volume profile fallback | Explicit SharpDX `OnRender` over immutable snapshots; current per-level context labels use priority and rectangle collision placement |
...
| `OrcaPriceAction` | No | Primary chart series only | None | Completed-bar FVG/iFVG, volume imbalance, causal pivots/structure, rejection blocks, and typed S-OB/C-OB/PB lifecycles; developing bars only shrink confirmed FVG geometry | No Tick Replay requirement; structural and lifecycle state is completed-bar driven | No shared or local order-flow cache | Explicit SharpDX `OnRender` over immutable zone/line/label arrays; no model calculation, cache access, or historical scan in render |
