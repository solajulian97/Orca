...
`Hollow Body Delta` is the opt-in quick-read alternative. It reserves a narrow center column through every populated high-to-low footprint row; no wick or OHLC spine is drawn through the delta labels. Every row shows centered strict `Ask - Bid`. Rows intersecting the open-to-close body receive transparent delta-colored outlines. Wick rows remain free-floating and use a faint neutral separator beneath the value so adjacent prices stay legible without implying candle body. Positive, negative and zero/unknown rows reuse Bid x Ask Positive, Negative and Neutral colors. Opacity is normalized across the candle's displayed rows from Bid x Ask Min Opacity to Max Opacity, so the strongest absolute row delta is brightest.

The automatic width is sized for an ungrouped signed four-digit value such as `-9999`; this removes the previous excess side space. Candle Width remains a larger user-selected minimum, Candle/Profile Gap remains outside the column, and Auto number formatting can compact values that exceed the available width.
